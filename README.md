...
📦src
 ...
 ┗ 📜reportWebVitals.js
```
이번 프로젝트에서는 **components, hooks, api, store** 등 기능을 나누는 구조를 사용하여 역할별로 폴더 구조를 구성하여 보았습니다. 
각 파일의 목적이 명확하게 구분되어 있어 개발 흐름을 이해하기 쉽고, 재사용성과 유지보수에도 효율적이라 초기 학습과 프로젝트 관리에 적합하다고 판단했습니다.
개발 중 API, 훅, 전역 상태 등에 빠르게 접근할 수 있어 기능 수정 시 효율적입니다.
**Zustand** 상태 관리 라이브러리를 도입하여 **store** 폴더를 통해 전역 상태를 체계적으로 분리하고 관리했습니다.
...
<li>로그아웃 버튼을 누르면 session storage에 저장된 엑세스 토큰을 삭제되며 로그아웃</li><br>
...
<li>검색 결과가 페이지네이션 변경에 따라 지도에 음식점이 표시</li><br>
...
<li>모달 밖의 화면을 클릭하거나 우측 상단의 close 버튼 클릭 시 모달 닫힘</li><br>
...
## **회고**
이번 프로젝트는 프론트엔드뿐 아니라 백엔드 서버도 직접 구축하여 **API** 통신을 구현하는 경험을 목표로 진행되었다.<br>
팀 프로젝트 경험이 없는 상황에서, 프론트와 백엔드가 어떻게 상호작용하며 데이터가 어떤 흐름으로 처리되는지 전체적인 구조를 이해하고자 **Express**를 사용해 서버를 구성하고 클라이언트와의 통신을 구현했다.<br>
직접 서버를 구축하면서 API 설계와 데이터 흐름에 대한 실질적인 감각을 익혔고, 프론트엔드와 백엔드 간의 소통 방식에 대한 고민도 함께할 수 있었다.<br>
다만 개인 프로젝트의 한계로 인해 협업 상황에서의 버전 관리, 코드 통합, 배포와 같은 실무 과정은 체험하지 못한 점은 아쉬움으로 남는다.<br>
그럼에도 불구하고, 이번 프로젝트를 통해 웹 애플리케이션이 동작하는 전반적인 구조를 이해하고 **Express** 기반의 서버 개발에 대한 기본기를 다질 수 있었던 의미 있는 시간이었다.<br>
...
## **개선사항**
<li>검색 기능이 API를 통해서만 이루어지는 현재 구조를 보완하기 위해, 공공데이터에 기반한 음식점 데이터베이스를 추가하여 검색 범위를 확대 필요</li>
<li>다양한 디바이스에서도 최적의 사용 경험을 제공할 수 있도록 반응형 UI를 수정 및 보완 필요</li>
<li>음식점 외에도 편의점, 카페 등 다양한 업종으로 검색 범위를 세분화하여 사용자 기능을 보다 구체화 필요</li>
